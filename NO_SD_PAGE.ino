...
void internalHTMLsend(EthernetClient cl){
  ...
  cl.println(F("<tr><td>Номер принимающего терминала ГОНЕЦ</td><td><input id=\"textField4\" type=\"text\" value=\"")); cl.print(toTerm); cl.println(F("\"></input></td></tr>"));
  String baudConfig = 
    "<tr><td>Скорость передачи данных (Бод)</td><td><select id=\"sel\">";
    uint32_t baudRates[] = { 9600, 19200, 38400, 57600, 115200 };
    for (byte i = 0; i < sizeof(baudRates) / sizeof(uint32_t); i++) {
      baudConfig += F("<option value=\"");
      baudConfig += i;
      baudConfig += F("\"");
      baudConfig += (Serial_MODE == i ? " selected" : "");
      baudConfig += F(">");
      baudConfig += baudRates[i];
      baudConfig += F("</option>");
    }
  cl.println(baudConfig);
  cl.println(F("</td></tr>"));
 // cl.println(F("<tr><td>Скорость передачи данных (Бод)</td><td><select id=\"sel\"><option value=\"0\">9600</option><option value=\"1\" selected>19200</option><option value=\"2\">38400</option><option value=\"3\">57600</option><option value=\"4\">115200</option></td></tr>"));
  cl.println(F("</table>"));
